...
def selection(User, Course, department):
    ...
    for i in sorted_tutors_list:
        if int(i["fitness"])>=int(User["fitness"]):
...
def crossover(User, totur_to_check):
    ...
    crossed_object.studentDays = User["daysRequired"]
    crossed_object.toturDays = totur_to_check["daysProvide"]
    
    check_student_congruence = User.get("daysRequired")
...
def fitness_calc(User):
    ...
    day_count = 0
    dep = 0
    if (User.get("department") == "software"): dep = 10
    aux = User.get("daysRequired")
    for i in aux:
        day_count+=1
        hours_count+= len(i.get("hours"))
...
#------------------------------------------------------#    
    Course = selection(User, course_needed, User["department"])
    decide_if_need_mutation(User,Course)
    terminate(User, Course)

def terminate(User, Course):
    print("Matching procces completed:\n")
    print("\nFor course: "+ Course + "\nUser: " + User["name"] + "\nFrom department: "+ User["department"] + "\nHas congruence with:\n")
    for i in crossed:
        print()
        print(i.toturID + " " + str(i.daysCongruence))
        print("\n")  
    
def decide_if_need_mutation(User,Course):
    if (len(crossed)==0):
        department_mutation(User,Course)
    else:
